...
def build_arcface_database():
    ...
    for folder_name in student_folders:
        ...
        if len(embeddings) < 3:
            print(f"  Not enough valid faces for {name} — skipping")
            continue
        ...
        for i in range(len(arr)):
            s = np.mean([
                float(np.dot(arr[i], arr[j]))
                for j in range(len(arr)) if i != j
            ])
            sims.append(s)

        mean_sim = np.mean(sims)
        kept     = [arr[i] for i, s in enumerate(sims)
                    if s >= mean_sim * 0.85]
        ...
        names_lines.append(f"{student_id},{name}")
        print(f"  Saved {student_id}.npy — {len(kept)}/{len(embeddings)} kept")

    names_path = os.path.join(config.EMBEDDINGS_DIR, 'names.txt')
    with open(names_path, 'w') as f:
        f.write('\n'.join(names_lines))

    print(f"\nSaved names.txt — {len(names_lines)} students")
    print("Done. ArcFace database built with antelopev2 (SCRFD + ResNet100)")
